...
def get_instance_info(client, db_instance_id):
    ...
    return arn, current_class, status
...
def wait_for_instance_ready(client, db_instance_id, desired_class):
    ...
    while True:
        ...
        arn, size, status = get_instance_info(client, db_instance_id)
    ...
    start_time_secs = time.time()
    # size changes take 10m but class changes can take more than 10m
    limit_secs = 1200
    sleep_time = 30
...
def resize_db_cluster(
    ...
    for member in members:
        ...
        arn, size, status = get_instance_info(client, dbinstance)
        if status != 'available':
